...
source "$LIB" 2>/dev/null || { echo "3 MySQL_Agent - mysql_common.sh not found"; exit 0; }
...
if [[ -n "$primary" ]] && [[ "$(scalar "$primary" "SELECT 1" 2>/dev/null)" == "1" ]]; then
    emit_checkmk 0 MySQL_Instance - "instance reachable via ${primary}"

    uptime="$(printf '%s' "SHOW GLOBAL STATUS LIKE 'Uptime'" | sqlx "$primary" 2>/dev/null | awk '{print $2}')"
    uptime="${uptime:-0}"; [[ "$uptime" =~ ^[0-9]+$ ]] || uptime=0
    emit_checkmk 0 MySQL_Uptime "uptime=${uptime}" "up $(( uptime / 86400 ))d (${uptime}s)"
    ...
    emit_checkmk "$st" MySQL_Connections "conn=${conn};;;0;${maxc}|used_pct=${pct};80;90" "${conn}/${maxc} connections (${pct}%)"
    ...
    if printf '%s' "$repl" | grep -qE 'IO_Running:'; then
        ...
        emit_checkmk "$st" "MySQL_Repl_${primary}" "lag=${lag:-0};${LAG_WARN};${LAG_CRIT}" "IO=${io:-?} SQL=${sqlt:-?} lag=${lag:-?}s"
    fi
else
    emit_checkmk 2 MySQL_Instance - "instance DOWN (no schema reachable)"
fi
...
for db in $DBS; do
    ...
    emit_checkmk "$st" "MySQL_Blocking_${db}" "blocked=${blk};1;10" "${blk} lock wait(s) in ${db}"
done
