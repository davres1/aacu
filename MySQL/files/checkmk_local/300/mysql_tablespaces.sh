...
source "$LIB" 2>/dev/null || { echo "3 MySQL_Tablespaces - mysql_common.sh not found"; exit 0; }
...
for db in $(list_databases); do
    ...
    # fullness is the meaningful "tablespace" signal in MySQL/MariaDB.
    ...
    emit_checkmk "$st" "MySQL_TS_${db}_datadir" "used_pct=${pct};${WARN};${CRIT}|size_mb=${size_mb}" "${db} datadir ${pct}% used, schema ${size_mb}MB"
done
...
df -P 2>/dev/null | awk 'NR>1 && $1 !~ /tmpfs|devtmpfs|overlay/ {gsub(/%/,"",$5); print $6"|"$5}' \
  | while IFS='|' read -r mount used; do
      ...
      emit_checkmk "$st" "MySQL_FS_${mount}" "used_pct=${used};85;95" "${mount} ${used}% used"
  done
