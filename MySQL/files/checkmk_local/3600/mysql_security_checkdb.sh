...
source "$LIB" 2>/dev/null || { echo "3 MySQL_Security - mysql_common.sh not found"; exit 0; }
...
for db in $(list_databases); do
    ...
    emit_checkmk "$st" "MySQL_Security_${db}" "issues=${issues};1;5|admin=${admin}|public=${pub}" "${issues} security issue(s)"
done
...
PY
    ...
    emit_checkmk "$st" "MySQL_CheckDB" "errors=${errors}|failed=${failed}" "CHECK TABLE: ${errors} error(s), ${failed} failed"
else
    emit_checkmk 3 "MySQL_CheckDB" - "no cached CHECK TABLE result yet"
fi
