#!/usr/bin/env bash
# GetCheckDBStatus.sh — fast, read-only retriever of the cached mysqlcheck /
# CHECK TABLE result written by CheckDB.sh. Adds an age_hours field. Safe to
...
if not os.path.exists(path):
    print(json.dumps({"available": False, "age_hours": None,
                      "hint": "CHECK TABLE has not run yet (see CheckDB.sh / scheduled cron)"}))
    raise SystemExit(0)
try:
