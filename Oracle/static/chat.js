...
async function send(message) {
  ...
  try {
    const resp = await fetch('/api/chat', {
      ...
      body: JSON.stringify({
        message,
        database: serverPicker.value || null,   // selected DB from the dropdown
      }),
    });
...
// Database picker — populated from /api/databases. Each option is an Oracle
// TNS alias from databases.ini. The chosen DB drives every chat message:
// the backend resolves its ansible_servername automatically.
...
async function loadServers() {
  try {
    ...
    for (const d of dbs) {
      ...
      opt.value = d.name;
      // Show DB name + which host it lives on, dim if the server isn't in
      // the Ansible inventory (so misconfig is visible at the dropdown level).
