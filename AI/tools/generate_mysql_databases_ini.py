#!/usr/bin/env python3
"""
generate_databases_ini.py — MySQL / MariaDB edition
---------------------------------------------------

Walks every MySQL host in the Ansible inventory, pulls each host's
`ansible_local.db_inventory` fact (produced by MySQL/files/db_inventory.sh
deployed at /etc/ansible/facts.d/db_inventory.fact), and writes
MySQL/inventory/databases.ini with one [<SCHEMA_NAME>] section per discovered
schema/database.

  * `ansible_servername` is set from the host that reported the DB.
  * `mysql_instance` is the instance the DB lives in (host\\instance).
...
Usage:
    python3 generate_mysql_databases_ini.py
    python3 generate_mysql_databases_ini.py --from-tree /tmp/facts --dry-run
"""
...
# MySQL system schemas — excluded by default (like SQL Server's system DBs).
SYSTEM_DBS: set[str] = {
...
MANAGED_FIELDS = ("ansible_servername", "mysql_instance", "database",
...
def extract_databases(per_host_facts: dict, include_system: bool) -> list[dict]:
    """db_inventory.sh emits:
       ansible_local.db_inventory.mysql = {
           "<host>\\<instance>": {
...
def merge_into_ini(existing_path: Path, db_rows: list[dict]) -> tuple[configparser.ConfigParser, dict]:
    ...
    # Rebuild [mysql_servers] roster.
    hosts = set()
    if parser.has_section("mysql_servers"):
        for key in parser["mysql_servers"]:
    ...
    if parser.has_section("mysql_servers"):
        parser.remove_section("mysql_servers")
    parser.add_section("mysql_servers")
    for h in sorted(hosts):
        parser.set("mysql_servers", h, None)
    ...
    for row in db_rows:
        ...
        parser.set(matched, "mysql_instance",     row["mysql_instance"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--group", default="mysql_servers",
    ...
    ap.add_argument("--out", default="MySQL/inventory/databases.ini", type=Path,
    ...
    if args.from_tree:
        ...
        tmp = tempfile.mkdtemp(prefix="mysql_facts_")
        try:
