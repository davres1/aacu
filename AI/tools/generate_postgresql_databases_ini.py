#!/usr/bin/env python3
"""
generate_postgresql_databases_ini.py — PostgreSQL edition
----------------------------------------------------------

Walks every PostgreSQL host in the Ansible inventory, pulls each host's
`ansible_local.db_inventory` fact (produced by PostgreSQL/files/db_inventory.sh
deployed at /etc/ansible/facts.d/db_inventory.fact), and writes
PostgreSQL/inventory/databases.ini with one [<DATABASE_NAME>] section per
discovered database.

  * `ansible_servername` is set from the host that reported the DB.
  * `pg_instance` is the cluster (host:port) the DB lives in.
  * Existing sections (connuser, connpass, dbhost, dbport, retention,
    emaillist, ...) are PRESERVED — re-running picks up newly added DBs
...
Usage:
    python3 generate_postgresql_databases_ini.py
    python3 generate_postgresql_databases_ini.py --from-tree /tmp/facts --dry-run
"""
...
# PostgreSQL system databases — excluded by default.
SYSTEM_DBS: set[str] = {
...
USER_OWNED_FIELDS = (
    "connuser", "connpass", "dbhost", "dbport",
    "environment", "emaillist", "retention",
)

# Fields the generator always (re)writes.
MANAGED_FIELDS = ("ansible_servername", "pg_instance", "database",
                  "version", "encoding", "lastupdated")
...
def extract_databases(per_host_facts: dict, include_system: bool) -> list[dict]:
    """db_inventory.sh emits:
       ansible_local.db_inventory.postgresql = {
           "<host>:<port>": {
               "instance_name": "...",  "version": ...,  "encoding": "...",
    ...
    for host, facts in per_host_facts.items():
        ...
        postgresql = inv.get("postgresql") or {}
        if not isinstance(postgresql, dict):
            continue

        for instance_key, instance_info in postgresql.items():
            ...
            encoding      = instance_info.get("encoding", "UTF8")

            for db in instance_info.get("databases", []) or []:
                name = (db.get("name") or "").strip()
                if not name:
                    continue
                if not include_system and name.lower() in SYSTEM_DBS:
                    continue
                rows.append({
                    "name":        name,
                    "host":        host_hint,
                    "pg_instance": instance_name,
                    "database":    name,
                    "version":     version,
                    "encoding":    encoding,
...
def merge_into_ini(existing_path: Path, db_rows: list[dict]) -> tuple[configparser.ConfigParser, dict]:
    ...
    # Rebuild [postgresql_servers] roster.
    hosts = set()
    if parser.has_section("postgresql_servers"):
        for key in parser["postgresql_servers"]:
    ...
    if parser.has_section("postgresql_servers"):
        parser.remove_section("postgresql_servers")
    parser.add_section("postgresql_servers")
    for h in sorted(hosts):
        parser.set("postgresql_servers", h, None)

    # Handle name collisions across hosts.
    for row in db_rows:
        candidate = row["name"]
        ...
        parser.set(matched, "pg_instance",        row["pg_instance"])
        parser.set(matched, "database",           row["database"])
        parser.set(matched, "version",            row["version"])
        parser.set(matched, "encoding",           row["encoding"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--group", default="postgresql_servers",
    ...
    ap.add_argument("--out", default="PostgreSQL/inventory/databases.ini", type=Path,
    ...
    ap.add_argument("--include-system-dbs", action="store_true",
                    help="Also emit sections for system databases (postgres, template0, template1).")
    ...
    if args.from_tree:
        ...
        tmp = tempfile.mkdtemp(prefix="pg_facts_")
        try:
