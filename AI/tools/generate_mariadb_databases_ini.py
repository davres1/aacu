#!/usr/bin/env python3
"""
generate_databases_ini.py — MariaDB edition
---------------------------------------------------

Walks every MariaDB host in the Ansible inventory, pulls each host's
`ansible_local.db_inventory` fact (produced by MariaDB/files/db_inventory.sh
deployed at /etc/ansible/facts.d/db_inventory.fact), and writes
MariaDB/inventory/databases.ini with one [<SCHEMA_NAME>] section per discovered
schema/database.

  * `ansible_servername` is set from the host that reported the DB.
  * `mariadb_instance` is the instance the DB lives in (host\\instance).
...
Usage:
    python3 generate_mariadb_databases_ini.py
    python3 generate_mariadb_databases_ini.py --from-tree /tmp/facts --dry-run
"""
...
# MariaDB system schemas — excluded by default (like SQL Server's system DBs).
SYSTEM_DBS: set[str] = {
...
MANAGED_FIELDS = ("ansible_servername", "mariadb_instance", "database",
...
def extract_databases(per_host_facts: dict, include_system: bool) -> list[dict]:
    """db_inventory.sh emits:
       ansible_local.db_inventory.mariadb = {
           "<host>\\<instance>": {
    ...
    for host, facts in per_host_facts.items():
        ...
        mysql = inv.get("mariadb") or {}
        if not isinstance(mysql, dict):
            continue

        for instance_key, instance_info in mysql.items():
            ...
            for db in instance_info.get("databases", []) or []:
                ...
                rows.append({
                    "name":           name,
                    "host":           host_hint,
                    "mariadb_instance": instance_name,
                    "database":       name,
                    "version":        version,
                    "edition":        edition,
...
def merge_into_ini(existing_path: Path, db_rows: list[dict]) -> tuple[configparser.ConfigParser, dict]:
    ...
    # Rebuild [mariadb_servers] roster.
    hosts = set()
    if parser.has_section("mariadb_servers"):
        for key in parser["mariadb_servers"]:
    ...
    if parser.has_section("mariadb_servers"):
        parser.remove_section("mariadb_servers")
    parser.add_section("mariadb_servers")
    for h in sorted(hosts):
        parser.set("mariadb_servers", h, None)

    # Handle name collisions across hosts: if "appdb" exists on two hosts,
    # the second one gets `appdb__<host>` as its section header.
    for row in db_rows:
        candidate = row["name"]
        ...
        parser.set(matched, "mariadb_instance",     row["mariadb_instance"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--group", default="mariadb_servers",
    ...
    ap.add_argument("--out", default="MariaDB/inventory/databases.ini", type=Path,
    ...
    if args.from_tree:
        ...
        tmp = tempfile.mkdtemp(prefix="mariadb_facts_")
        try:
