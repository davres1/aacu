"""
Run a read-only MySQL / MariaDB SQL query against a target host via Ansible.
...
def run_query(server, database, raw_query):
    ...
    if not database:
        return {
            ...
            "error": "MySQL requires a schema/database name — none supplied.",
        }
    ...
    extra_vars = {
        ...
        "databases_ini":  settings.MYSQL_DATABASES_INI,
        "mysql_user":     settings.MYSQL_OS_USER,
    }

    out = ansible_runner.run_playbook("mysql/run_sql_query.yml", server, extra_vars=extra_vars)
