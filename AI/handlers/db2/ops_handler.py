"""
Operational actions for Db2 (LUW) hosts. Each handler runs an Ansible playbook
that ships a single shell script (under Db2/files/*.sh) to the target host and
captures its JSON output:

  - check_blocking_locks  -> DetectBlockingLocks.sh
  - add_datafile_space    -> ALTER TABLESPACE ... (extend a DMS tablespace)
  - health_check          -> CheckDb2Status.sh + db_inventory.sh
  ...
  - tempdb_status         -> MonitorTablespaces.sh (system temp subset)
  - security_audit        -> SecurityAudit.sh
  - patch_level           -> PatchLevelCheck.sh
  - alwayson_status       -> MonitorHADR.sh (HADR role + log gap)
"""
...
def _parse_json_tail(stdout):
    """Each Db2 script emits a final compact JSON line on stdout."""
    if not stdout:
        return None
    text = stdout.strip()
...
def check_blocking_locks(server, database=None):
    ...
    out = ansible_runner.run_playbook(
        "db2/check_blocking_locks.yml", server,
...
def add_datafile_space(server, database, tablespace, add_mb):
    """Extend a DMS tablespace by add_mb. Uses ALTER TABLESPACE ... RESIZE/EXTEND."""
    ...
    out = ansible_runner.run_playbook(
        "db2/add_datafile_space.yml", server,
        extra_vars={
            "target_host":  server,
...
def health_check(server):
    out = ansible_runner.run_playbook(
        "db2/health_check.yml", server,
        extra_vars={"target_host": server},
    )
    status    = ansible_runner.extract_task_result(out, "Run CheckDb2Status.sh") or {}
    ...
    return {
        ...
        "_ansible_status":    _ansible_meta(out, "Run CheckDb2Status.sh", status),
...
def backup_status(server):
    return _run_script(server, "db2/verify_backups.yml",     "Run VerifyBackups.sh")

def integrity_status(server):
    return _run_script(server, "db2/dbcc_checkdb.yml",       "Run GetCheckDBStatus.sh")

def disk_status(server):
    return _run_script(server, "db2/monitor_disk_space.yml", "Run MonitorTablespaces.sh")
...
def agent_jobs(server, lookback_hours=None):
    ...
    return _run_script(server, "db2/monitor_agent_jobs.yml", "Run MonitorJobs.sh", extra)

def tempdb_status(server):
    # MonitorTablespaces.sh also covers system temporary tablespaces — filtered client-side.
    return _run_script(server, "db2/monitor_tempdb.yml", "Run MonitorTablespaces.sh")

def security_audit(server):
    return _run_script(server, "db2/security_audit.yml",     "Run SecurityAudit.sh")

def patch_level(server):
    return _run_script(server, "db2/patch_level.yml",        "Run PatchLevelCheck.sh")
