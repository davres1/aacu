"""
Operational actions for MariaDB / MariaDB hosts. Each handler runs an Ansible
playbook that ships a single shell script (under MariaDB/files/*.sh) to the target
host and captures its JSON output:
  ...
  - health_check          -> CheckMariaDBStatus.sh + db_inventory.sh
...
def _parse_json_tail(stdout):
    """Each MariaDB script emits a final compact JSON line on stdout."""
    if not stdout:
        return None
    text = stdout.strip()
...
def check_blocking_locks(server, database=None):
    ...
    out = ansible_runner.run_playbook(
        "mariadb/check_blocking_locks.yml", server,
...
def add_datafile_space(server, database, tablespace, add_mb):
    ...
    MariaDB InnoDB file-per-table datafiles autoextend, so for a general
    ...
    out = ansible_runner.run_playbook(
        "mariadb/add_datafile_space.yml", server,
        extra_vars={
            "target_host":    server,
...
def health_check(server):
    out = ansible_runner.run_playbook(
        "mariadb/health_check.yml", server,
        extra_vars={"target_host": server},
    )
    status    = ansible_runner.extract_task_result(out, "Run CheckMariaDBStatus.sh") or {}
    ...
    return {
        ...
        "_ansible_status":    _ansible_meta(out, "Run CheckMariaDBStatus.sh", status),
...
def backup_status(server):
    return _run_script(server, "mariadb/verify_backups.yml",     "Run VerifyBackups.sh")

def integrity_status(server):
    return _run_script(server, "mariadb/dbcc_checkdb.yml",       "Run GetCheckDBStatus.sh")

def disk_status(server):
    return _run_script(server, "mariadb/monitor_disk_space.yml", "Run MonitorTablespaces.sh")
...
def agent_jobs(server, lookback_hours=None):
    ...
    return _run_script(server, "mariadb/monitor_agent_jobs.yml", "Run MonitorJobs.sh", extra)

def tempdb_status(server):
    # MonitorTablespaces.sh also covers temp / undo tablespaces — filtered client-side.
    return _run_script(server, "mariadb/monitor_tempdb.yml", "Run MonitorTablespaces.sh")

def security_audit(server):
    return _run_script(server, "mariadb/security_audit.yml",     "Run SecurityAudit.sh")

def patch_level(server):
    return _run_script(server, "mariadb/patch_level.yml",        "Run PatchLevelCheck.sh")

def alwayson_status(server):
    """MariaDB replication — same intent name as the SQL Server side for chat parity."""
    return _run_script(server, "mariadb/monitor_alwayson.yml",   "Run MonitorReplication.sh")

def performance_review(server):
    return _run_script(server, "mariadb/performance_review.yml", "Run PerformanceReview.sh")
