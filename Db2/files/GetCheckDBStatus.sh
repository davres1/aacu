#!/usr/bin/env bash
# GetCheckDBStatus.sh — fast, read-only retriever of the cached INSPECT CHECK
# result written by CheckDB.sh. Adds an age_hours field. Safe to call from the
# chatbot (no heavy work).
#
...
if not os.path.exists(path):
    print(json.dumps({"available": False, "age_hours": None,
                      "hint": "INSPECT CHECK has not run yet (see CheckDB.sh / weekly cron)"}))
    raise SystemExit(0)
try:
