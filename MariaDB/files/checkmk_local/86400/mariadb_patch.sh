#!/usr/bin/env bash
# mysql_patch.sh — CheckMK local check (daily): MariaDB/MariaDB server version and
...
source "$LIB" 2>/dev/null || { echo "3 MariaDB_Version - mariadb_common.sh not found"; exit 0; }
...
emit_checkmk 0 "MariaDB_Version" - "version=${VERSION:-unknown} (${COMMENT:-?})"
