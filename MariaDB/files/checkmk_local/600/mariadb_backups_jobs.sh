...
source "$LIB" 2>/dev/null || { echo "3 MariaDB_Backups - mariadb_common.sh not found"; exit 0; }
...
for db in $(list_databases); do
    ...
    if [[ -z "$newest" ]]; then
        emit_checkmk 2 "MariaDB_Backup_Full_${db}" - "no backup file found for ${db} under ${bdir}"
    else
        age_h=$(( ( $(date +%s) - ${newest%.*} ) / 3600 ))
        st=0; (( age_h >= FULL_MAX_H )) && st=1; (( age_h >= FULL_CRIT_H )) && st=2
        emit_checkmk "$st" "MariaDB_Backup_Full_${db}" "age_h=${age_h};${FULL_MAX_H};${FULL_CRIT_H}" "last backup ${age_h}h ago (${bdir})"
    fi
    ...
    emit_checkmk "$st" "MariaDB_Events_${db}" "disabled=${disabled};1;5" "${disabled} disabled/errored event(s) in ${db}"
done
