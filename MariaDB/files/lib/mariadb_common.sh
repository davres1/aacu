# mariadb_common.sh — shared helpers for every MariaDB / MariaDB DBA script.
# Source this at the top of each script:  source "$(dirname "$0")/lib/mariadb_common.sh"
#
...
RESERVED = {'mariadb_servers', 'DEFAULT'}
for s in p.sections():
    if s in RESERVED:
        continue
